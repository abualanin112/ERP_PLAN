...
- **[`04-operations/INFRASTRUCTURE_AND_RESILIENCE.md`](./04-operations/INFRASTRUCTURE_AND_RESILIENCE.md):** Bootstrap sequence, reverse graceful shutdown handlers, and cache resilience.
- **[`04-operations/WORKERS_AND_CRON.md`](./04-operations/WORKERS_AND_CRON.md):** Ephemeral cron schedules, distributed PostgreSQL locks, and task execution contexts.
...
2. **[`04-operations/INFRASTRUCTURE_AND_RESILIENCE.md`](./04-operations/INFRASTRUCTURE_AND_RESILIENCE.md):** Master graceful shutdowns and system resilience.
...
- **The Caching Fallback Invariant:** Caching logic must use local memory bounds (LRU) without disrupting active database transactions or filling pod memory.
...
| **Resilience & Caching** | **High**               | Database reconnects, local `LRUCache`, and 10s graceful shutdown timers active. |
...
flowchart TD
    ...
    subgraph Phase4 [4. Distributed Eventual Consistency]
        M4[Offload non-critical tasks to queues (e.g., PgBoss), scale independent worker containers]
    end
